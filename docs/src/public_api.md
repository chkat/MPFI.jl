...
For `x::BigInterval` and integer `p`, `x^p` uses [`Base.power_by_squaring`](https://docs.julialang.org/en/v1/base/math/#Base.power_by_squaring). **Squaring steps** call `square` (C `mpfi_sqr`), not interval multiplication `x * x`. 

!!! note "Squaring is not the same as multiplying an interval by itself"
    In interval arithmetic, $x * y$ computes an enclosure of $\{a\cdot b : a \in x, b \in y\}$. For a single interval $x$, writing $x * x$ treats the two factors as **independent** copies of the set, which **widens** the result. For example, `BigInterval(-2, 2) * BigInterval(-2, 2)` gives $[-4, 4]$, while `square(BigInterval(-2, 2))` gives $[0, 4]$, which is the actual range of $t^2$ for $t \in [-2, 2]$. The exported `power_by_squaring` matches `x^p` and uses `square` for those squaring steps.
